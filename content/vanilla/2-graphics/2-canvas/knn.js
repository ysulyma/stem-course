...
function wireUpButtons() {
    for (const btn of $$(".color-selection")) {
...
function wireUpCanvas() {
    const drawingCanvas = $("#layer-drawing");
...
function redrawPoints() {
    ...
    // draw the points
    for (let x = config.granularity; x < dotsCanvas.width; x += config.granularity) {
        for (let y = config.granularity; y < dotsCanvas.height; y += config.granularity) {
            const nearestNeighbors = findNearestNeighbors({ x, y }, state.points, state.k);
            const colorCounts = nearestNeighbors.reduce((acc, curr) => {
                if (!acc[curr.color]) {
                    acc[curr.color] = 0;
                }
                acc[curr.color]++;
 ...
 */
function findNearestNeighbors(pt, points, k) {
    const log = (...args) => {
    ...
    let nearestNeighborsWithDistances = [];
