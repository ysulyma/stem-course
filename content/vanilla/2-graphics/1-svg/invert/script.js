...
const dragEvents = {
    ...
    move: (_e, hit) => {
        ...
        let [, svgY] = screenToSVG(svg, hit.x, hit.y);
        svgY = clamp({ max: 5, min: -5, value: svgY });
        // update
        Object.assign(state, {
            showX: false,
            y: -svgY,
    ...
    return $svg("g", {}, [
        ...
        ...range(xMin, xMax + 1).map((n) => n !== 0 &&
            $svg("g", {}, [
                ...
                $svg("text", { class: "axis-label", dx: tickSize, dy: 0.5, x: n, y: 0 }, format(n)),
            ])),
        // origin
        $svg("text", { class: "axis-label", dx: 0.25, dy: 0.4, x: 0, y: 0 }, "0"),
        // horizontal grid lines
        ...range(yMin, yMax + 1).map((n) => n !== 0 &&
            $svg("g", {}, [
                $svg("line", {
                    class: "gridline",
                    x1: xMin,
                ...
                $svg("text", { class: "axis-label", dx: 0.4, dy: 0.2, x: 0, y: -n }, format(n)),
            ])),
        $svg("line", {
            class: "major-axis",
            x1: xMin,
