...
function updateGraphToggle() {
    ...
    $("#toggle-graph > circle").setAttribute("cx", String(settings.showGraph ? 20 : 60));
}
...
function text(node) {
    ...
    const finalLatex = node.foldChildren(node.ctrlSeq || "", (latex, child) => latex + text(child));
    return finalLatex;
}
