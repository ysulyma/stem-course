...
export function findLines(code: string) {
...
export function disableLines(...lines) {
	...
	const preventDelete = (view) => {
		...
		for (let i = start; i <= end; ++i) {
			...
			if (
				dom.node.nodeType === dom.node.ELEMENT_NODE &&
				dom.node.classList.contains("cm-disabled")
			) {
				return true;
			}
		}
			...
			class {
				...
				update(desc, tr) {
					...
					for (const r of desc.changedRanges) {
						...
						for (let i = 0; i < lines.length; ++i) {
							if (lines[i] > fromB) lines[i] += fromA - toA - (fromB - toB);
						}
						this.setDecorations();
					}
				}
...
const freezeMark = Decoration.line({
	attributes: { class: "cm-disabled", contentEditable: false },
});
